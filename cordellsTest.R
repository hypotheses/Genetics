...
cordellsTest <- 
function (mat.snp, cl, cov, genes = NULL, warnError = TRUE) 
{
  ...
  for (i in 1:n.combs) {
    ...
    x2 <- x[, combs[i, 2]]
    z2 <- z[, combs[i, 2]]
    woIA <- try(glm(cl ~ x1 + z1 + x2 + z2 + cov , family = "binomial"), 
                silent = TRUE)
    if (is(woIA, "try-error")) {
      ll.main[i] <- NA
      vec.error[[i]] <- woIA
      cat("NOTE: A warning has occurred when testing the SNPs in columns ", 
          combs[i, 1], " and ", combs[i, 2], " of mat.snp.\n", 
          "Therefore, the likelihood and ", "the test statistic is set to NA.\n", 
          "(For this warning, see vec.error[[", i, "]] in the output of colCordell.)\n\n", 
          sep = "")
    }
    else ll.main[i] <- woIA$deviance/-2
    full <- try(glm(cl ~ x1 + z1 + x2 + z2 + x1 * x2 + x1 * 
      z2 + z1 * x2 + z1 * z2 + cov, family = "binomial"), silent = TRUE)
    if (is(full, "try-error")) {
      ll.full[i] <- NA
      vec.error[[i]] <- full
      cat("NOTE: A warning has occurred when testing the SNPs in columns ", 
          combs[i, 1], " and ", combs[i, 2], " of mat.snp.\n", 
          "Therefore, the likelihood and ", "the test statistic are set to NA.\n", 
          "(For this warning, see vec.error[[", i, "]] in the output of colCordell.)\n\n", 
          sep = "")
    }
    else ll.full[i] <- full$deviance/-2
  }
  if (warnError) 
    options(warn = wa)
  ...
  names(ll.full) <- names(stat) <- names(pval) <- names(ll.main) <- names(vec.error) <- paste(snpnames[combs[, 
                                                                                                             1]], snpnames[combs[, 2]], sep = " : ")
  if (!is.null(genes)) 
    genecombs <- paste(genes[combs[, 1]], genes[combs[, 2]], 
                       sep = " : ")
  else genecombs <- NULL
  out <- list(ll.main = ll.main, ll.full = ll.full, stat = stat, 
              pval = pval, genes = genecombs, vec.error = vec.error)
  class(out) <- "colEpi"
  out
}

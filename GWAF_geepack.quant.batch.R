...
## geepack.quant.batch
geepack.quant.batch
function (phenfile, genfile, pedfile, phen, model = "a", covars = NULL, 
          outfile, col.names = T, sep.ped = ",", sep.phe = ",", sep.gen = ",") 
{
  ...
  read.in.data <- function(phenfile, genfile, pedfile, sep.ped = sep.ped, 
                           sep.phe = sep.phe, sep.gen = sep.gen) {
    print("Reading in Data")
    ped.dat <- read.table(genfile, header = TRUE, na.strings = "", 
                          sep = sep.gen)
  ...
  phensnp.dat <- read.in.data(phenfile, genfile, pedfile, sep.ped = sep.ped, 
                              sep.phe = sep.phe, sep.gen = sep.gen)
  ...
  if (!is.null(covars) & sum(snplist %in% covars) >= 1) {
    names(test.dat)[which(names(test.dat) %in% paste(snplist[snplist %in% 
    ...
    if (sum(colinear) > 0) {
      if (model %in% c("a", "r", "d")) 
        result <- rbind(result, c(phen, i, count1, rep(NA, 
                                                       6)))
      else result <- rbind(result, c(phen, i, count1, rep(NA, 
                                                          10)))
    }
    else {
      if (sort(count1)[1] + sort(count1)[2] < 10 | (count1[3] < 
        10 & model == "r")) {
        if (model %in% c("a", "r", "d")) 
        ...
        if (model == "a") {
          if (is.null(covars)) 
            gee.test.v <- try(geese(test2.dat[, phen] ~ 
              test2.dat[, i], data = test2.dat, id = famid, 
                                    corstr = "independence"))
          else gee.test.v <- try(geese(test2.dat[, phen] ~ 
            test2.dat[, i] + x.covar, data = test2.dat, 
                                       id = famid, corstr = "independence"))
          if (class(gee.test.v) != "try-error") {
            gee.test <- summary(gee.test.v)
            if (class(gee.test) != "try-error" & gee.test$error == 
              0) 
              tmp <- c(gee.test$mean[2, 1:3], 1, "additive", 
                       pchisq(gee.test$mean[2, "wald"], 1, lower.tail = F))
            else tmp <- rep(NA, 6)
          }
          else tmp <- rep(NA, 6)
        }
          ...
          if (min(count1) < 10) {
            snp.i <- test2.dat[, i]
            snp.i[snp.i == 2] <- 1
            assign("snp.i", snp.i, envir = .GlobalEnv, 
                   inherits = T)
            if (is.null(covars)) 
              gee.test.v <- try(geese(test2.dat[, phen] ~ 
                snp.i, data = test2.dat, id = famid, 
                                      corstr = "independence"))
            else gee.test.v <- try(geese(test2.dat[, 
                                                   phen] ~ snp.i + x.covar, data = test2.dat, 
                                         id = famid, corstr = "independence"))
            if (class(gee.test.v) != "try-error") {
              gee.test <- summary(gee.test.v)
              if (class(gee.test) != "try-error" & gee.test$error == 
                0) 
                tmp <- c(gee.test$mean[2, 1], NA, NA, 
                         gee.test$mean[2, 2], NA, NA, gee.test$mean[2, 
                                                                    3], 1, "dominant", pchisq(gee.test$mean[2, 
                                                                                                            3], 1, lower.tail = F))
              else tmp <- rep(NA, 10)
            }
            else tmp <- rep(NA, 10)
            ...
            if (is.null(covars)) 
              gee.test.v <- try(geese(test2.dat[, phen] ~ 
                x.snp, data = test2.dat, id = famid, 
                                      corstr = "independence"))
            else gee.test.v <- try(geese(test2.dat[, 
                                                   phen] ~ x.snp + x.covar, data = test2.dat, 
                                         id = famid, corstr = "independence"))
            if (class(gee.test.v) != "try-error") {
              gee.test <- summary(gee.test.v)
              if (class(gee.test.v) != "try-error" & 
                gee.test$error == 0) {
                chisq = try(t(matrix(gee.test$mean[c("x.snp1", 
                                                     "x.snp2"), 1], ncol = 1)) %*% solve(gee.test.v$vbeta[names(gee.test.v$beta) %in% 
                                                       c("x.snp1", "x.snp2"), names(gee.test.v$beta) %in% 
                                                       c("x.snp1", "x.snp2")]) %*% matrix(gee.test$mean[c("x.snp1", 
                                                                                                          "x.snp2"), 1], ncol = 1))
                tmp <- c(gee.test$mean[2:3, 1], gee.test$mean[3, 
                                                              1] - gee.test$mean[2, 1], gee.test$mean["x.snp1", 
                                                                                                      2], gee.test$mean["x.snp2", 2], sqrt(gee.test.v$vbeta[names(gee.test.v$beta) == 
                                                                                                        "x.snp1", names(gee.test.v$beta) == 
                                                                                                        "x.snp1"] + gee.test.v$vbeta[names(gee.test.v$beta) == 
                                                                                                        "x.snp2", names(gee.test.v$beta) == 
                                                                                                        "x.snp2"] - 2 * gee.test.v$vbeta[names(gee.test.v$beta) == 
                                                                                                        "x.snp1", names(gee.test.v$beta) == 
                                                                                                        "x.snp2"]), chisq, 2, "general", pchisq(chisq, 
                                                                                                                                                2, lower.tail = F))
              }
              else tmp <- rep(NA, 10)
            }
          ...
          if (is.null(covars)) 
            gee.test.v <- try(geese(test2.dat[, phen] ~ 
              snp.i, data = test2.dat, id = famid, corstr = "independence"))
          else gee.test.v <- try(geese(test2.dat[, phen] ~ 
            snp.i + x.covar, data = test2.dat, id = famid, 
                                       corstr = "independence"))
          if (class(gee.test.v) != "try-error") {
            gee.test <- summary(gee.test.v)
            if (class(gee.test) != "try-error" & gee.test$error == 
              0) 
              tmp <- c(gee.test$mean[2, 1:3], 1, "dominant", 
                       pchisq(gee.test$mean[2, "wald"], 1, lower.tail = F))
            else tmp <- rep(NA, 6)
          }
          else tmp <- rep(NA, 6)
          ...
          if (class(gee.test.v) != "try-error") {
            ...
            if (class(gee.test) != "try-error" & gee.test$error == 
              0) 
              tmp <- c(gee.test$mean[2, 1:3], 1, "recessive", 
                       pchisq(gee.test$mean[2, "wald"], 1, lower.tail = F))
            else tmp <- rep(NA, 6)
          }
          else tmp <- rep(NA, 6)
  ...
  if (model %in% c("a", "d", "r")) {
    colnames(result) <- c("phen", "snp", "n0", "n1", "n2", 
                          "beta", "se", "chisq", "df", "model", "pval")
  }
  else colnames(result) <- c("phen", "snp", "n0", "n1", "n2", 
                             "beta10", "beta20", "beta21", "se10", "se20", "se21", 
                             "chisq", "df", "model", "pval")
  write.table(result, outfile, quote = F, row.names = F, col.names = col.names, 
              sep = ",", na = "", append = T)
}
## GWAF Version 2.0
## lmepack.batch function
lmepack.batch <- 
function (phenfile, genfile, pedfile, phen, kinmat, model = "a", 
          ...
          sep.gen = ",") 
{
  ...
  for (i in snplist) {
    ...
    if (is.null(covars)) 
      lme.cov.out <- try(lmekin(test2.dat[, phen] ~ 1, 
                                random = ~1 | id, varlist = kmat, na.action = na.omit))
    else lme.cov.out <- try(lmekin(test2.dat[, phen] ~ x.covar, 
                                   random = ~1 | id, varlist = kmat, na.action = na.omit))
        ...
        if (model == "a") {
          ...
          if (min(count1) < 10) {
            ...
            if (class(lme.out) != "try-error") {
              ...
              tmp <- c(max(v.cov - sum(lme.out$theta), 
                           0)/var(test2.dat[, phen]), lme.out$ctable[2, 
                                                                     1], NA, NA, sqrt(lme.out$var[2, 2]), 
                       NA, NA, chisq, 1, "dominant", pchisq(chisq, 
                                                            1, lower.tail = F))
            }
            else tmp <- rep(NA, 11)
          }

plot.pedigree <- function (x, id = x$id, status = x$status, affected = x$affected, 
...
{ 
  ...
  ## A modified function from plot.pedigree found in kinship2
  ## adding str=-90 and change adj=c(1,0.5) to rotate the ID label
  Call <- match.call()
  n <- length(x$id)
  if (is.null(status)) 
    status <- rep(0, n)

...
addterm.lm <-
           ...
           k = 2, sorted = FALSE, ...)
  {
    ...
    if(missing(scope) || is.null(scope)) stop("no terms in scope")
    aod <- add1.lm(object, scope=scope, scale=scale)[ , -4L]
...
addterm.glm <-
           ...
           k = 2, sorted = FALSE, trace = FALSE, ...)
  {
    ...
    ## ---- foreach experiment ---- ##
    Z <- foreach:::foreach ( tt = scope,.combine='rbind',.inorder=TRUE) %dopar% {
...
#     }
    ## ---- foreach experiment ---- ##
    Z <- foreach:::foreach ( i = 1L:ns,.combine='rbind',.inorder=TRUE) %dopar% {
      if(trace) {
        message("trying -", scope[i])
        utils::flush.console()
      }
                     ...
                     x = NULL, k = 2, ...)
{
  ...
  Z <- foreach:::foreach ( tt = scope,.combine='rbind',.inorder=TRUE) %dopar% {
    ...
    usex <- match(asgn, match(stt, sTerms), 0L) > 0L
    X <- x[, usex|ousex, drop = FALSE]
    z <-  glm.fit(X, y, wt, offset=offset,family=object$family, control=object$control)
    if (! test=="Rao") {
      zout <- c(z$rank,z$deviance)
    } else {
      ## WLS for score test (comes out as model SS)
      zz <- glm.fit(X, r, w, offset=offset)
      zout <-c(z$rank,z$deviance,zz$null.deviance - zz$deviance)
    }
  } 
  RSS[-1] <- Z[,2]
  dfs[-1] <- Z[,1]
  if ( test == "Rao" ) score[-1] <- Z[,3]
  ## ---- foreach experiment ---- ##    
  if (scale == 0)
    dispersion <- summary(object, dispersion = NULL)$dispersion
  else dispersion <- scale

...
ggplot(data=data.frame(x=c(0:25))) + geom_point(size=10, aes(x=x,y=x,shape=x)) + facet_wrap(
  ~ x, scales='free')   + xlab('')   + ylab('')   +	scale_shape_identity()   +	opts(
    axis.text.x=theme_blank(), axis.text.y=theme_blank(), axis.ticks=theme_blank(), legend.position='none')

ggsave('symbols.png')

#Line types
ggplot(data=data.frame(x=c(1:6))) + 
...
ggsave('linetypes.png')

#Colors
df = data.frame(x=rep(1:26, 26), 
                y=rep(1:26, each=26))
df$c = NA
...
df$text = ifelse(apply(df[,c('r','g','b')], 1, sum) > (255*3/2),
                 'black', 'white')
...
ggplot(df, aes(x=x, y=y, fill=c, label=n)) + 
  geom_tile() + 
  geom_text(aes(colour=text)) + 	
  scale_fill_identity() +	
...
ggsave('colors.png')
ggplot(df[1:100,], 
       aes(x=1, y=n, fill=c, label=hex2, colour=text)) +	

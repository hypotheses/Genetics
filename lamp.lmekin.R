lamp.lmekin <- function(pedigree="/research/imp_data/users/bhoom/blackR21.fam",phendata=phenotype,lampdata,k=500,genostart=2,
                        genoend=ncol(lampdata),outcomes,
                        famid="FID",covar="",suffix="",outdir="./lampResult",chr,kinmat,...) {
  ##genotype is "pedigree" here
  ...
  for (outcome in outcomes) {
    ...
    ## FIX
    cat("SNP,EFFANC,SEANC,TANC,PANC\n",sep="",file=outfilename)
    p.LME <- data.frame(SNP=character(0),
    ...
    while (begin <= genoend) {
      ...
      for (j in 1:allsnp) {
        ...
        mf <- as.formula(paste(outcome,"~",anc,covar))
        datfm <- as.formula(paste(outcome, "~+IID+",anc,covar))
...
#            rSNP <- SLK$ctable[2,]
      ...
      rm(SLK,data,snp,mysnp,mf,datfm,gwdat,freq,rst,p.LME)
      p.LME <- data.frame(SNP=character(0),
                          EFFANC=double(0),SEANC=double(0),TANC=double(0),PANC=double(0),
                          stringsAsFactors=F)
